...
MIN_NW_F = 1000  # the larger the faster
NUM_ROWS_MORE_OR_LESS = 5_000  # may not return results if this is too small
...
def process(_q, _iolock, _nw_id2row_ids, _test_word_windows, min_target, max_target):
    best_cei = 0.0
    ...
    while True:
        ...
        if from_q is None:
            ...
            with _iolock:
                ...
                print(f'would result in num test windows={_best__num_rows:,}')
        ...
        if min_target < len(_row_ids) < max_target:
...
# target num rows - to balance windows with found next-words equally across halves of corpus
# WARNING: use FULL test-word-windows, before removing windows with infrequent next-words
target_num_rows = len(test_word_windows) // 2
min_target = target_num_rows - NUM_ROWS_MORE_OR_LESS
max_target = target_num_rows + NUM_ROWS_MORE_OR_LESS
...
pool = mp.Pool(configs.Constants.num_processes,
               initializer=process,
               initargs=(q, iolock, nw_id2row_ids, test_word_windows, min_target, max_target))
# raise SystemExit
